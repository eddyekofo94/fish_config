function gss_old -d "fzf git status"
    ...
    set -l enter_cmd "$git_stage_unstage {2}"
    ...
    eval "$git_unstaged_files" | fzf \
        ...
        --bind='ctrl-b:+change-preview(git blame --color-by-age {2})' \
        --bind="ctrl-s:transform:[$FZF_PROMPT != '$prompt_add' ] && echo '$mode_reset' || echo '$mode_add'" \
        --bind="enter:execute($git_stage_unstage {2})" \
